...
export function renderProjects(project, containerElement, headingLevel = 'h2') {
  ...
  if (!validHeadingTags.includes(headingLevel)) {
    console.warn(`Invalid heading level "${headingLevel}" provided. Falling back to 'h2'.`);
    headingLevel = 'h2';
  }
  ...
  article.innerHTML = `
    ...
    <img src="${project.image || 'placeholder.jpg'}" alt="${project.title || 'Image Unavailable'}">
...
export async function fetchJSON(url) {
  try {
    console.log(`Fetching data from: ${url}`); // Log the URL to check if it's correct
    const response = await fetch(url);
